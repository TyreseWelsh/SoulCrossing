...
#include "../Plugins/GameplayStateMachine/Source/GameplayStateMachine/Public/GameplayStateManagerComponent.h"
...
AMinion::AMinion()
{
	...
	StateManagerComponent = CreateDefaultSubobject<UGameplayStateManagerComponent>(TEXT("StateManager"));
...
void AMinion::MontageBeginBlendOut(UAnimMontage* Montage, bool bInterrupted)
{
	...
	{
		bCanInput = true;
		Possessed = true;																// Setting to true to move out of inactive state in ANIMATION blueprint

		if (StateManagerComponent != nullptr)
		{
			StateManagerComponent->SwitchStateByKey("Idle");							// Used to switch to the skeletons gameplay logic Idle state
		}
	}
}
